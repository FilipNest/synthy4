    ...
    makeRequest("/status")
        .then(data => {
            ...
            for (let user in users) {
                ...
                document.querySelector(`[data-location="${user.location}"]`).innerHTML = mapping[user.note] + "<br/>" + (user.beat + 1).toString().padStart(2, "0");

                if (user.id === you.id) {

                    document.querySelector(`[data-location="${user.location}"]`).setAttribute("data-you", "true");
            ...
            if (play.length) {
                ...
                play.forEach((n, i) => {
                    ...
                    let square = document.querySelector(`[data-location="${n.location}"`).setAttribute("data-playing", "true")
...
let moveDirection = (direction) => {
    ...
    let target = document.querySelector(`[data-row="${targetRow}"][data-column="${targetColumn}"]`);
...
let beat = function(beatNumber) {

    makeRequest("/beat?" + beatNumber);
...
let getLinked = (users) => {
    ...
    for (let userID in users) {
        ...
        if (userRow === yourRow || userRow === yourRow - 1 || userRow === yourRow + 1) {

            // Then check adjacent columns

            if (userColumn === yourColumn || userColumn === yourColumn - 1 || userColumn === yourColumn + 1) {

                notes.push({ "note": user.note, "beat": user.beat, location: user.location });
